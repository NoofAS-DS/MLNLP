...
st.set_page_config(page_title="Job Level Prediction", layout="wide")
...
def load_data():
    ...
    raw_df = pd.read_csv(BytesIO(resp_raw.content))

    resp_prep = requests.get(PREP_URL)
    resp_prep.raise_for_status()
    prep_df = pd.read_csv(BytesIO(resp_prep.content))
